...
def patch_manifest(m):
    ...
    # Localhost matches added back to every WAR entry so the local proxy's
    # redirect page can navigate to chrome-extension:// URLs.
  ...
  if (!_optionsPromise && (force || Date.now() - _updateAt > 1000 * 3600)) {
    _optionsPromise = new Promise(async (resolve) => {
      ...
      try {
        ...
        // CRITICAL: Only write to chrome.storage from the SW context.
        // Writing from window/sidepanel fires chrome.storage.onChanged.
        // The extension's useStorageState hook listens to onChanged and calls
        // setState. If that fires during React's render phase -> #185 loop.
        // The SW (first context to load) already wrote these values on startup.
        // Sidepanel context must NEVER write to chrome.storage -- reads only.
        try {
          if (chrome?.storage?.local?.set && identity && !globalThis.window) {
...
// FIX: iat MUST be static (1700000000). Dynamic iat (Math.floor(Date.now()/1000))
// causes useStorageState to see a new token value on every sidepanel refresh,
// fires the onChanged listener, feeds the setState loop -> React error #185.
if (chrome?.storage?.local?.get) {
...
if (globalThis.window) {
  ...
  if (location.pathname == "/sidepanel.html" && location.search == "") {
    ...
    // The SW bootstrap sets sidepanelToken async -- without the retry we race
    // it and open a needless OAuth tab every cold start, causing the login loop.
    let _spAttempts = 0
